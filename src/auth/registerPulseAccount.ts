...
export async function registerPulseAccount(credential: {
  ...
  try {
    ...
    if (isLikelyCorsError(e)) {
      throw new PulseAuthError(
        "Browser blocked the request (CORS). On PythonAnywhere, allow Origin http://localhost:5173 and POST /auth/google with Content-Type application/json. See README.",
      );
    }
    throw new PulseAuthError(
