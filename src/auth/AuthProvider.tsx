  ...
  const onGoogleCredential = useCallback(
    async (idToken: string) => {
      ...
      try {
        ...
        if (!clientId) {
          showToast(t("auth.errorGoogleClientId"));
          return;
        }
        ...
        try {
          ...
          showToast(t("auth.errorPulseSave", { message: msg }));
          return;
        }
        ...
        showToast(message || t("auth.errorGoogleFailed"));
        console.error(error);
      } finally {
        setIsSigningIn(false);
      }
    },
    [isSigningIn, persistSession, syncMeFromApi, showToast, t],
  );

  const onGoogleError = useCallback(() => {
    showToast(t("auth.errorGoogleCancelled"));
  }, [showToast, t]);
